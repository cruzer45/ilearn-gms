SELECT `Student`.`stuID`,`Student`.`stuFirstName`,`Student`.`stuLastName`,`Student`.`stuClsCode`, `Subject`.subName,`Subject`.`subStaffCode`, `Subject`.`subCredits`,`Grade`.*, COALESCE(SUM(`Demerits`.`demerits`),0) AS 'demerits',COALESCE(sum(`RollCall`.`rolAbsent`),0)  AS 'rolAbsent', COALESCE(sum(`RollCall`.`rolTardy`),0) AS 'rolLate', `Term`.`trmShortName`, `clsHomeRoom` , `graAvgMid`,`graAvgGPA`
FROM `Grade`
...
WHERE `Grade`.graStatus = 'Active' 
		...
		AND `Student`.`stuClsCode` LIKE '3B'
