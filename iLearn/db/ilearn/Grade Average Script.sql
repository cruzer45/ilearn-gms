SELECT `stuID`,`subCode`,  `stuClsCode`, CONCAT_WS(', ',`stuLastName`,`stuFirstName`) AS 'Student',`subName`, ((SUM(`grdPointsEarned`)/ SUM(`Assments`.assmtTotalPoints)) * 100) AS 'Grade'
FROM `TermGrade`
...
WHERE `grdStatus` = 'Active' AND `grdPointsEarned` != 'Excused'  AND `subCode` LIKE '3%PE' AND `assmtType` != 'Exam'
